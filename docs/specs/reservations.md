...
- courante, adossée à Valkey : `backend/app/routers/reservations_valkey.py`, modèle
  `ValkeyReservationCreate`/`ValkeyReservation` (`backend/app/models/reservation.py:31-69`).

Ce spec documente principalement l'implémentation Valkey (source de vérité actuelle),
plus le flux iCal legacy qui reste actif.

## Modèle de données

### `ValkeyReservation` (`backend/app/models/reservation.py:31-49`)
...
best-effort — `backend/app/services/valkey_service.py:799-833`).

### Clés Valkey (préfixe DT via `_key`, `backend/app/services/valkey_service.py:54-64` →
`f"{dt}:{':'.join(parts)}"`)
- `{dt}:reservations:{id}` — document JSON de la réservation (`valkey_service.py:836-837`).
- `{dt}:reservations:index` — SET de tous les IDs de réservation (`valkey_service.py:840`).
- `{dt}:reservations:by_date:{YYYY-MM-DD}` — SET d'IDs, un membre par jour couvert par
  `[debut.date(), fin.date()]` inclus (fan-out un ajout par jour, `valkey_service.py:842-848`).
- `{dt}:reservations:by_vehicle:{immat}` — SET d'IDs pour un véhicule (`valkey_service.py:851-852`).

Le fan-out `by_date` est linéaire au nombre de jours couverts — une réservation de N
jours écrit N clés `sadd` (`valkey_service.py:845-848`). `(inferred — verify)` : pas de
borne sur N, un `fin` très éloigné de `debut` peut générer un grand nombre d'écritures.

## Entrées / sorties

Préfixe Valkey : `/api/calendar/{dt}/reservations` (`reservations_valkey.py:19-22`).
...
|---|---|---|---|
| GET | `/api/calendar/{dt}/reservations` | utilisateur authentifié | Liste filtrable par `from`, `to`, `vehicule_immat` (`reservations_valkey.py:25-64`) |
| POST | `/api/calendar/{dt}/reservations` | utilisateur authentifié | Création (`reservations_valkey.py:67-130`) |
| GET | `/api/calendar/{dt}/reservations/{id}` | utilisateur authentifié | Détail (`reservations_valkey.py:133-161`) |
| PUT | `/api/calendar/{dt}/reservations/{id}` | créateur OU rôle `Gestionnaire DT` | Modification (`reservations_valkey.py:164-250`) |
| DELETE | `/api/calendar/{dt}/reservations/{id}` | créateur OU rôle `Gestionnaire DT` | Annulation (`reservations_valkey.py:253-300`) |
...
1. `fin` doit être strictement postérieur à `debut`, sinon `400` — vérifié à la création
   (`reservations_valkey.py:91-95`) et à la modification (`reservations_valkey.py:206-210`).
2. Le véhicule (`vehicule_immat`) doit exister (`valkey.get_vehicle`), sinon `404`
   (`reservations_valkey.py:97-103`, `212-218`).
3. Le chauffeur (`chauffeur_nivol`) doit exister (`valkey.get_benevole`), sinon `404`
   (`reservations_valkey.py:105-111`, `220-226`).
4. **Chevauchement** : deux réservations chevauchent ssi `debut < autre.fin AND fin >
   autre.debut` (`valkey_service.py:1172`) — comparaison stricte, donc deux réservations
...
5. Le chevauchement est vérifié **uniquement pour le même véhicule** — deux réservations
   ...
   (`test_reservations_valkey.py::TestReservationOverlapValidation::test_overlap_different_vehicles`).
6. À la modification, la réservation courante est exclue de la vérification de
   chevauchement via `exclude_id` (`valkey_service.py:961-966`, `1167-1168`).
7. Modification/suppression : autorisées seulement si `created_by == current_user.email`
   ...
   (`reservations_valkey.py:199-203`, `282-286`). Comparaison de rôle en chaîne exacte —
...
8. La création/modification tente une synchronisation Google Calendar best-effort si une
   ...
   d'être créée/modifiée (`valkey_service.py:808-833`, `994-1034`).

## Flux iCal

- Deux endpoints publics (`ical.py:101`, `162`) génèrent un `.ics` à partir des
  événements **Google Calendar** (`calendar_service.get_events`), pas de la source Valkey
  ...
  toutes les réservations Valkey natives.
- Cache Valkey/Redis clé `ical:{dt}:all` ou `ical:{dt}:vehicle:{immat_normalisé}`, TTL 60s
...
- Réservation multi-jours : indexée sous une clé `by_date` par jour couvert ; suppression
  symétrique dans `delete_reservation` (`valkey_service.py:1126-1133`) et
  `update_reservation` (`valkey_service.py:1050-1067`).
- `list_reservations` sans filtre de date utilise des bornes par défaut
  `2020-01-01`–`2030-12-31` lors d'un filtre partiel (`valkey_service.py:900-905`) —
...
- Sync Google Calendar : configuration absente ou `redis.json()` non supporté (ex.
  ...
  (`valkey_service.py:801-806`, `986-992`, `1094-1099`).
- `check_reservation_overlap` recharge toutes les réservations du véhicule via
  `list_reservations` puis filtre en mémoire (`valkey_service.py:1162-1175`) —
...
| # | Critère | Test |
|---|---|---|
| 1 | Création basique | `test_reservations_valkey.py::TestReservationCRUD::test_create_reservation` |
| 2 | Lecture par ID | `test_reservations_valkey.py::TestReservationCRUD::test_get_reservation` |
| 3 | Lecture ID inexistant → `None` | `test_reservations_valkey.py::TestReservationCRUD::test_get_nonexistent_reservation` |
| 4 | Liste triée par `debut` | `test_reservations_valkey.py::TestReservationCRUD::test_list_reservations` |
| 5 | Filtre par plage de dates | `test_reservations_valkey.py::TestReservationCRUD::test_list_reservations_by_date_range` |
| 6 | Filtre par véhicule | `test_reservations_valkey.py::TestReservationCRUD::test_list_reservations_by_vehicle` |
| 7 | Modification | `test_reservations_valkey.py::TestReservationCRUD::test_update_reservation` |
| 8 | Suppression + nettoyage index | `test_reservations_valkey.py::TestReservationCRUD::test_delete_reservation` |
| 9 | Pas de chevauchement si créneaux disjoints | `test_reservations_valkey.py::TestReservationOverlapValidation::test_no_overlap_different_times` |
| 10 | Rejet si chevauchement total | `test_reservations_valkey.py::TestReservationOverlapValidation::test_overlap_same_time` |
| 11 | Rejet si chevauchement partiel (début) | `test_reservations_valkey.py::TestReservationOverlapValidation::test_overlap_partial_start` |
| 12 | Rejet si chevauchement partiel (fin) | `test_reservations_valkey.py::TestReservationOverlapValidation::test_overlap_partial_end` |
| 13 | Modif n'entre pas en conflit avec soi-même | `test_reservations_valkey.py::TestReservationOverlapValidation::test_update_no_overlap_with_self` |
| 14 | Chevauchement ignoré si véhicules différents | `test_reservations_valkey.py::TestReservationOverlapValidation::test_overlap_different_vehicles` |
| 15 | Permissions update/delete (créateur/Gestionnaire DT) | **NON COUVERT** (aucun test HTTP sur `reservations_valkey.py:199-203`/`282-286`) |
...
1. **Deux modèles de réservation coexistent** : le legacy `Reservation` lié à Google
   ...
   `ValkeyReservation` (`reservation.py:31-69`). Le frontend ne possède une interface
   TypeScript que pour le **legacy** ; la forme Valkey n'en a aucune. Migration
   inachevée : `f206ac9` (2026-03-15) a remplacé l'intégration Google Calendar par des
   réservations natives Valkey sans retirer l'ancien modèle.
