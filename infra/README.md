...
1. **Terraform** installed (>= 1.0)`# macOS
...
1. **Generate Service Account Key**:`gcloud iam service-accounts keys create clef-backend-dev-key.json \
  --iam-account=clef-backend@rcq-fr-dev.iam.gserviceaccount.com \
  --project=rcq-fr-dev`
2. **Set Secret Values**:`# GOOGLE_CLIENT_ID
echo -n "your-google-client-id" | gcloud secrets versions add GOOGLE_CLIENT_ID \
  --data-file=- --project=rcq-fr-dev

# GOOGLE_CLIENT_SECRET
echo -n "your-google-client-secret" | gcloud secrets versions add GOOGLE_CLIENT_SECRET \
  --data-file=- --project=rcq-fr-dev
