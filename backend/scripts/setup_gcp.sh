...
KMS_KEYRING_ID=$(tofu output -raw kms_keyring_id 2>/dev/null || echo "")
REGION=$(printf '%s' "$KMS_KEYRING_ID" | sed -n 's|projects/[^/]*/locations/\([^/]*\)/keyRings/.*|\1|p')
if [ -z "$REGION" ]; then
...
GCP_REGION=${REGION:-europe-west9}
