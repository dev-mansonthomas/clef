"""Tests for RedisService multi-tenant operations."""
import pytest
...
@pytest_asyncio.fixture
async def redis_dt92(redis_client) -> RedisService:
    """Create RedisService for DT92."""
    return RedisService(redis_client=redis_client, dt="DT92")
...
class TestRedisServiceConfiguration:
    ...
    @pytest.mark.asyncio
    async def test_set_and_get_configuration(self, redis_dt75):
        ...
        success = await redis_dt75.set_configuration(config)
        assert success is True
        
        retrieved = await redis_dt75.get_configuration()
...
class TestRedisServiceVehicles:
    """Test vehicle operations."""
    
    @pytest.mark.asyncio
    async def test_set_and_get_vehicle(self, redis_dt75):
        ...
        success = await redis_dt75.set_vehicle(vehicle)
        assert success is True
        
        retrieved = await redis_dt75.get_vehicle("AB-123-CD")
        assert retrieved is not None
    ...
    @pytest.mark.asyncio
    async def test_list_vehicles(self, redis_dt75):
        ...
        vehicles = await redis_dt75.list_vehicles()
        assert len(vehicles) == 2
        assert "AB-123-CD" in vehicles
        assert "EF-456-GH" in vehicles

    @pytest.mark.asyncio
    async def test_multi_tenant_isolation(self, redis_dt75, redis_dt92):
        ...
        await redis_dt75.set_vehicle(vehicle_dt75)
        await redis_dt92.set_vehicle(vehicle_dt92)
        
        # DT75 should only see its vehicle
        dt75_vehicles = await redis_dt75.list_vehicles()
        ...
        dt92_vehicles = await redis_dt92.list_vehicles()
        assert len(dt92_vehicles) == 1
        assert "IJ-789-KL" in dt92_vehicles
        assert "AB-123-CD" not in dt92_vehicles


class TestRedisServiceBenevoles:
    """Test bénévole operations."""

    @pytest.mark.asyncio
    async def test_set_and_get_benevole(self, redis_dt75):
        ...
        success = await redis_dt75.set_benevole(benevole)
        assert success is True

        retrieved = await redis_dt75.get_benevole("123456")
        assert retrieved is not None
    ...
    @pytest.mark.asyncio
    async def test_list_benevoles_by_ul(self, redis_dt75):
        ...
        await redis_dt75.set_benevole(benevole1)
        await redis_dt75.set_benevole(benevole2)
        await redis_dt75.set_benevole(benevole3)

        # List all
        all_benevoles = await redis_dt75.list_benevoles()
        assert len(all_benevoles) == 3

        # List UL 81 only
        ul81_benevoles = await redis_dt75.list_benevoles(ul="81")
...
class TestRedisServiceCarnet:
    """Test carnet de bord operations."""

    @pytest.mark.asyncio
    async def test_add_and_get_carnet_entries(self, redis_dt75):
        ...
        await redis_dt75.add_carnet_entry(entry1)
        await redis_dt75.add_carnet_entry(entry2)

        entries = await redis_dt75.get_carnet_entries("AB-123-CD")
    ...
    @pytest.mark.asyncio
    async def test_get_latest_carnet_entry(self, redis_dt75):
        ...
        latest = await redis_dt75.get_latest_carnet_entry("AB-123-CD")
        assert latest is not None
        assert latest.type == "Retour"

        latest_prise = await redis_dt75.get_latest_carnet_entry("AB-123-CD", entry_type="Prise")
