...
class TestReminderService:
    ...
    async def test_no_overdue_devis(self):
        ...
        mock_redis.get_configuration = AsyncMock(return_value=None)
        mock_redis.list_vehicles = AsyncMock(return_value=["AB-123-CD"])
        recent_date = datetime.utcnow() - timedelta(days=1)
        dossier = _make_dossier("REP-2026-001", devis=[
            _make_devis("1", date_envoi=recent_date),
        ])
        mock_redis.list_dossiers_reparation = AsyncMock(return_value=[dossier])

        svc = ReminderService(mock_redis)
    ...
    async def test_custom_delay_from_config(self):
        ...
        mock_redis = AsyncMock()
        mock_redis.get_configuration = AsyncMock(return_value=config)
